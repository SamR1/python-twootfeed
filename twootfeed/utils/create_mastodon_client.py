...
if __name__ == '__main__':
    ...
    print('Registering a new app with {url} called {app_name} and saving '
          'credentials in {client_id_file}'.format(**mast_cfg))

    Mastodon.create_app(
        mast_cfg['app_name'],
        api_base_url=mast_cfg['url'],
        to_file='twootfeed/' + mast_cfg['client_id_file'],
        scopes=['read']
    )
    mastodon = Mastodon(
        client_id='twootfeed/' + mast_cfg['client_id_file'],
        api_base_url=mast_cfg['url']
    )
    ...
    mastodon.log_in(
        user_email,
        password,
        to_file='twootfeed/' + mast_cfg['access_token_file'],
        scopes=['read']
    )
    ...
    try:
        ...
        print('Credentials look good; client reports user\'s account name is: '
              + res['acct'])
        print('Configuration complete; app should appear at: '
              + mast_cfg['url']
              + '/oauth/authorized_applications')
        print('You should not need to log in again unless this app is '
